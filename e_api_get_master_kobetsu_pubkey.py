...
FNAME_OUTPUT = 'master_' + S_TARGET_CLMID +'.csv'
...
##                    str_text = str_text + next(iter_keys) + ','
                ...
                for i in range(len(list_return)):
                    ...
                    if str_sTargetCLMID == 'CLMIssueSizyouMstKabu' :
                        if list_return[i].get('sZyouzyouSizyou') == '9' :
                            bool_judge = False
                    
                    # 先物、OP     過去の限月を削除する
                    if str_sTargetCLMID == 'CLMIssueMstSak' \
                       or str_sTargetCLMID == 'CLMIssueMstOp' :
...
if __name__ == "__main__":
    ...
    dic_req_item = {
        ...
        'sCLMID':           'CLMMfdsGetMasterData',     # マスタ情報問合取得（項目別のマスターデータ取得）
        'sTargetCLMID':     S_TARGET_CLMID,             # 対象機能ＩＤ
        'sTargetColumn':    my_sTargetColumn,           # 各マスタデータで取得する項目名
