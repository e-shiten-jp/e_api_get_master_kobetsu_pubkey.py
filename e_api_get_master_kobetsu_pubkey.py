...
# 機能: 1.株式銘柄マスタ問合取得（CLMStkGetIssueMstKabu）用 列名を漢字に変換 
# 引数: 列名
# 返値: 漢字名 string型
...
# 機能: 2.株式銘柄市場マスタ問合取得（CLMStkGetIssueSizyouMstKabu）用 列名を漢字に変換 
# 引数: 列名
# 返値: 漢字名 string型
...
# 機能: 3.先物銘柄マスタ問合取得（CLMStkGetIssueMstSak）用 列名を漢字に変換 
# 引数: 列名
# 返値: 漢字名 string型
...
# 機能: 4.オプション銘柄マスタ問合取得（CLMStkGetIssueMstOp）用 列名を漢字に変換 
# 引数: 列名
# 返値: 漢字名 string型
...
# 機能: 5.指数銘柄マスタ問合取得、6.為替銘柄マスタ問合取得（CLMIssueMstOther）用 列名を漢字に変換 
# 引数: 列名
# 返値: 漢字名 string型
...
# 機能: 7.日付情報問合取得（CLMStkGetDateZyouhou）用 列名を漢字に変換 
# 引数: 列名
# 返値: 漢字名 string型
...
# 機能: 8.呼値情報問合取得（CLMStkGetYobine）用 列名を漢字に変換 
# 引数: 列名
# 返値: 漢字名 string型
...
# 機能: 9.代用掛目情報問合取得（CLMStkGetDaiyouKakeme）用 列名を漢字に変換 
# 引数: 列名
# 返値: 漢字名 string型
...
# 10.株式銘柄別・市場別規制情報問合取得
def func_column_kanji_CLMStkGetIssueSizyouKiseiKabu(str_column_name):
    if str_column_name == 'sIssueCode' : str_name_kanji = '銘柄コード'          # 例:6501
    elif str_column_name == 'sZyouzyouSizyou' : str_name_kanji = '上場市場'     # 00：東証
    elif str_column_name == 'sTeisiKubun' : str_name_kanji = '停止区分'         # 0:通常（無）
                                                                                # 1:取引禁止
                                                                                # 2:成行禁止
                                                                                # 3:端株禁止
    elif str_column_name == 'sGenbutuKaituke' : str_name_kanji = '現物/買付'    # sTeisiKubun 参照
    elif str_column_name == 'sGenbutuKaitukeYoku' : str_name_kanji = '現物/買付（翌営業日）'    # sTeisiKubun 参照
    elif str_column_name == 'sGenbutuUrituke' : str_name_kanji = '現物/売付'                    # sTeisiKubun 参照
    elif str_column_name == 'sGenbutuUritukeYoku' : str_name_kanji = '現物/売付（翌営業日）'    # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouSinkiKaitate' : str_name_kanji = '制度信用/買建'       # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouSinkiKaitateYoku' : str_name_kanji = '制度信用/買建（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouSinkiUritate' : str_name_kanji = '制度信用/売建'       # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouSinkiUritateYoku' : str_name_kanji = '制度信用/売建（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouKaiHensai' : str_name_kanji = '制度信用/買返済'        # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouKaiHensaiYoku' : str_name_kanji = '制度信用/買返済（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouUriHensai' : str_name_kanji = '制度信用/売返済'        # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouUriHensaiYoku' : str_name_kanji = '制度信用/売返済（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouGenbiki' : str_name_kanji = '制度信用/現引'            # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouGenbikiYoku' : str_name_kanji = '制度信用/現引（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouGenwatasi' : str_name_kanji = '制度信用/現渡'          # sTeisiKubun 参照
    elif str_column_name == 'sSeidoSinyouGenwatasiYoku' : str_name_kanji = '制度信用/現渡（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouSinkiKaitate' : str_name_kanji = '一般信用/買建'       # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouSinkiKaitateYoku' : str_name_kanji = '一般信用/買建（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouSinkiUritate' : str_name_kanji = '一般信用/売建'       # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouSinkiUritateYoku' : str_name_kanji = '一般信用/売建（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouKaiHensai' : str_name_kanji = '一般信用/買返済'        # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouKaiHensaiYoku' : str_name_kanji = '一般信用/買返済（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouUriHensai' : str_name_kanji = '一般信用/売返済'        # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouUriHensaiYoku' : str_name_kanji = '一般信用/売返済（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouGenbiki' : str_name_kanji = '一般信用/現引'            # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouGenbikiYoku' : str_name_kanji = '一般信用/現引（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouGenwatasi' : str_name_kanji = '一般信用/現渡'          # sTeisiKubun 参照
    elif str_column_name == 'sIppanSinyouGenwatasiYoku' : str_name_kanji = '一般信用/現渡（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sZizenCyouseiC' : str_name_kanji = '事前調整有無'      # 0：なし（無効）
                                                                                    # 1：あり（有効）
    elif str_column_name == 'sZizenCyouseiCYoku' : str_name_kanji = '事前調整有無（翌営業日）'  # sZizenCyouseiC 参照
    elif str_column_name == 'sSokuzituNyukinC' : str_name_kanji = '即日入金規制有無'            # sZizenCyouseiC 参照
    elif str_column_name == 'sSokuzituNyukinCYoku' : str_name_kanji = '即日入金規制有無（翌営業日）'     # sZizenCyouseiC 参照
    elif str_column_name == 'sSokuzituNyukinKiseiDate' : str_name_kanji = '即日入金規制日時'    # YYYYMMDDHHMMSS,00000000000000
    elif str_column_name == 'sSinyouSyutyuKubun' : str_name_kanji = '信用一極集中区分'          # 0：なし
                                                                                                # 1：あり
                                                                                                # 2：日々公表銘柄
    elif str_column_name == 'sSinyouSyutyuKubunYoku' : str_name_kanji = '信用一極集中区分（翌営業日）'     # sSinyouSyutyuKubun 参照
    else :
        str_name_kanji = str_column_name
    return str_name_kanji


# 11.派生銘柄別・市場別規制情報問合取得
def func_column_kanji_CLMStkGetIssueSizyouKiseiHasei(str_clumn_name):
    if str_column_name == 'sIssueCode' : str_name_kanji = '銘柄コード'      # 例:160060018
    elif str_column_name == 'sZyouzyouSizyou' : str_name_kanji = '上場市場'     # 01:東証
    elif str_column_name == 'sTeisiKubun' : str_name_kanji = '停止区分'     # 0：通常（無）
                                                                            # 1：取引禁止
                                                                            # 2：成行禁止
                                                                            # 3：端株禁止
    elif str_column_name == 'sKaitate' : str_name_kanji = '買建'            # sTeisiKubun 参照
    elif str_column_name == 'sKaitateYoku' : str_name_kanji = '買建（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sUritate' : str_name_kanji = '売建'            # sTeisiKubun 参照
    elif str_column_name == 'sUritateYoku' : str_name_kanji = '売建（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sKaiHensai' : str_name_kanji = '買返済'        # sTeisiKubun 参照
    elif str_column_name == 'sKaiHensaiYoku' : str_name_kanji = '買返済（翌営業日）'     # sTeisiKubun 参照
    elif str_column_name == 'sUriHensai' : str_name_kanji = '売返済'        # sTeisiKubun 参照
    elif str_column_name == 'sUriHensaiYoku' : str_name_kanji = '売返済（翌営業日）'     # sTeisiKubun 参照
    else :
        str_name_kanji = str_column_name
    return str_name_kanji


# 機能: 12.保証金マスタ情報問合取得（CLMStkGetHosyoukinMst）用 列名を漢字に変換 
# 引数: 列名
# 返値: 漢字名 string型
...
# 機能: 13.取引所エラー等理由コード情報問合取得（CLMStkGetOrderErrReason）用 列名を漢字に変換 
# 引数: 列名
# 返値: 漢字名 string型
def func_column_kanji_CLMStkGetOrderErrReason(str_column_name):
    if str_column_name == "sErrReasonCode" : str_name_kanji = "取引所エラー等理由コード"
    elif str_column_name == "sErrReasonText" : str_name_kanji = "取引所エラー等理由テキスト"
    else :
        str_name_kanji = str_column_name
    return str_name_kanji
...
def func_column_kanji(str_sTargetCLMID, str_clumn_name):
    str_column_kanji = ''
    if str_sTargetCLMID == 'CLMStkGetIssueMstKabu' :        # 1.株式銘柄マスタ問合取得
        str_column_kanji = func_column_kanji_CLMStkGetIssueMstKabu(str_clumn_name)
        
    elif str_sTargetCLMID == 'CLMStkGetIssueSizyouMstKabu' :    # 2.株式銘柄市場マスタ問合取得
        str_column_kanji = func_column_kanji_CLMStkGetIssueSizyouMstKabu(str_clumn_name)
        
    elif str_sTargetCLMID == 'CLMStkGetIssueMstSak' :       # 3.先物銘柄マスタ問合取得
        str_column_kanji = func_column_kanji_CLMStkGetIssueMstSak(str_clumn_name)
        
    elif str_sTargetCLMID == 'CLMStkGetIssueMstOp' :        # 4.オプション銘柄マスタ問合取得
        str_column_kanji = func_column_kanji_CLMStkGetIssueMstOp(str_clumn_name)
        
    elif str_sTargetCLMID == 'CLMStkGetIssueMstIndex' :     # 5.指数銘柄マスタ問合取得
        str_column_kanji = func_column_kanji_CLMIssueMstOther(str_clumn_name)

    elif str_sTargetCLMID == 'CLMStkGetIssueMstFx' :        # 6.為替銘柄マスタ問合取得
        str_column_kanji = func_column_kanji_CLMIssueMstOther(str_clumn_name)
    
    elif str_sTargetCLMID == 'CLMStkGetDateZyouhou' :       # 7.日付情報問合取得
        str_column_kanji = func_column_kanji_CLMStkGetDateZyouhou(str_clumn_name)
        
    elif str_sTargetCLMID == 'CLMStkGetYobine' :            # 8.呼値情報問合取得
        str_column_kanji = func_column_kanji_CLMStkGetYobine(str_clumn_name)

    elif str_sTargetCLMID == 'CLMStkGetDaiyouKakeme' :      # 9.代用掛目情報問合取得
        str_column_kanji = func_column_kanji_CLMStkGetDaiyouKakeme(str_clumn_name)
        
    elif str_sTargetCLMID == 'CLMStkGetIssueSizyouKiseiKabu' :  # 10.株式銘柄別・市場別規制情報問合取得
        str_column_kanji = func_column_kanji_CLMStkGetIssueSizyouKiseiKabu(str_clumn_name)
            
    elif str_sTargetCLMID == 'CLMStkGetIssueSizyouKiseiHasei' : # 11.派生銘柄別・市場別規制情報問合取得
        str_column_kanji = func_column_kanji_CLMStkGetIssueSizyouKiseiHasei(str_clumn_name)
            
    elif str_sTargetCLMID == 'CLMStkGetHosyoukinMst' :      # 12.保証金マスタ問合取得
        str_column_kanji = func_column_kanji_CLMStkGetHosyoukinMst(str_clumn_name)
        
    elif str_sTargetCLMID == 'CLMStkGetOrderErrReason' :    # 13.取引所エラー等理由コード情報問合取得
...
# 
def func_delete_past_gengetsu(list_data):
    bool_judge = True
                ...
                for i in range(len(list_return)):
                    ...
                    if str_sTargetCLMID == 'CLMStkGetIssueMstSak' \
                       or str_sTargetCLMID == 'CLMStkGetIssueMstOp' :
                        bool_judge = func_delete_past_gengetsu(list_return[i])
